...
export function EvidencePanel({ evidence, activeId, onSelect }: EvidencePanelProps) {
  return (
    <aside className="evidence-panel" aria-label="法规证据" data-lawagent-evidence-panel>
      ...
      <div className="evidence-summary" data-lawagent-evidence-summary><BookOpen size={18} /><div><b>{evidence.length || '—'} 条</b><span>已进入回答证据集</span></div></div>
      <div className="evidence-list" data-lawagent-evidence-list>
        ...
        {evidence.map((item, index) => (
          <button className={`evidence-card ${activeId === item.article_id ? 'active' : ''}`} type="button" key={item.article_id} data-lawagent-evidence-card={item.article_id} onClick={() => onSelect(item.article_id)}>
